...
const MediaGallery = ({ media }) => {
  ...
  // Helper to get a unique key for media items
  const getMediaKey = (url) => {
    return url || 'unknown';
  };
    ...
    if (media && media.length > 0) {
      media.forEach((item) => {
        if (item.type === 'image' || item.type === 'certificate' || item.type === 'screenshot') {
          const img = new Image();
          const key = getMediaKey(item.thumbnail || item.url);
          img.onload = () => handleImageLoad(key);
          img.onerror = () => handleImageLoad(key); // Mark as loaded even on error to remove placeholder
          img.src = item.thumbnail || item.url;
        } else if (item.type === 'video' && item.thumbnail) {
          const img = new Image();
          const key = getMediaKey(item.thumbnail);
          img.onload = () => handleImageLoad(key);
          img.onerror = () => handleImageLoad(key); // Mark as loaded even on error
  ...
  const handleImageClick = (index) => {
    ...
    const key = `lightbox-${getMediaKey(url)}`;

    // Preload the full-size image if not already loaded
    if (!imagesLoaded[key]) {
      const img = new Image();
      img.onload = () => handleImageLoad(key);
      img.onerror = () => handleImageLoad(key);
      img.src = url;
    }
    ...
    <div className="mt-6">
      ...
      >
          ...
          return (
            ...
            >
              ...
              <img
                ...
                className={`w-full h-full object-cover transition-all duration-500 group-hover:scale-110 ${imagesLoaded[key] ? 'opacity-100' : 'opacity-0'
                  }`}
                loading="lazy"
        ...
        {videos.map((video, index) => {
          const key = getMediaKey(video.thumbnail);
          return (
            <motion.div
              key={`video-${index}`}
              initial={{ opacity: 0, scale: 0.9 }}
            ...
            >
              ...
              <img
                ...
                className={`w-full h-full object-cover transition-transform duration-300 group-hover:scale-110 ${imagesLoaded[key] ? 'opacity-100' : 'opacity-0'
                  }`}
                loading="lazy"
                onLoad={() => handleImageLoad(key)}
                onError={() => handleImageLoad(key)}
          ...
          >
            ...
            >
              ...
              <div className="relative overflow-hidden rounded-lg">
                ...
                {!imagesLoaded[`lightbox-${getMediaKey(images[activeIndex].url)}`] && (
                ...
                >
                  <img
                    ...
                    onLoad={() => handleImageLoad(`lightbox-${getMediaKey(images[activeIndex].url)}`)}
                    onError={() => handleImageLoad(`lightbox-${getMediaKey(images[activeIndex].url)}`)}
                    onClick={(e) => {
                ...
                {images[activeIndex].url.includes('real-estate') && imagesLoaded[`lightbox-${getMediaKey(images[activeIndex].url)}`] && (
